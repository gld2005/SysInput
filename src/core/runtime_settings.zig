...
pub const Store = struct {
    ...
    pub fn initAt(allocator: std.mem.Allocator, directory: []const u8) !struct { store: Store, status: LoadStatus } {
        ...
        if (bytes) |contents| {
            ...
            flags = decoded.flags;
            status = .loaded;
            return .{ .store = .{ .allocator = allocator, .path = path, .temporary_path = temporary_path, .flags = std.atomic.Value(u16).init(flags), .abbreviation_prefix = std.atomic.Value(u8).init(decoded.prefix) }, .status = status };
        }
        return .{ .store = .{
            .allocator = allocator,
            .path = path,
    ...
    pub fn save(self: *Store) !void {
        ...
        const bytes = encodeWithPrefix(self.flags.load(.acquire), self.abbreviation_prefix.load(.acquire));
...
pub fn encode(flags: u16) [HEADER_SIZE + PAYLOAD_SIZE]u8 {
    return encodeWithPrefix(flags, ';');
}

fn encodeWithPrefix(flags: u16, prefix: u8) [HEADER_SIZE + PAYLOAD_SIZE]u8 {
...
const Decoded = struct { flags: u16, prefix: u8 };
fn decodeData(bytes: []const u8) !Decoded {
    ...
    if (!((version == 1 and payload_size == 2) or (version == VERSION and payload_size == PAYLOAD_SIZE))) return error.UnsupportedSettingsVersion;
    ...
    return .{ .flags = flags, .prefix = prefix };
}
